...
class CartaoCertoSupabaseUser extends BaseAuthUser {
  CartaoCertoSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> cartaoCertoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CartaoCertoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
